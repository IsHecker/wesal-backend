...
using Wesal.Application.Authorization;
...
namespace Wesal.Infrastructure.Authorization;
...
internal sealed class CustomClaimsTransformation(IServiceScopeFactory serviceScopeFactory) : IClaimsTransformation
{
    public async Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal)
    {
        ...
        if (result.IsFailure)
        {
            throw new WesalException(nameof(IPermissionService.GetUserPermissionsAsync), result.Error);
        }
